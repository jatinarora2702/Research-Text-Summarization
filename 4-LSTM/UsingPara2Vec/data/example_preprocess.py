...
def parse_line_to_example(name, abstract, article_rep, title):
    ...
    words.update(toks)
    nodocs[0] = nodocs[0] + 1
    indi_len[0] = max(indi_len[0], len(article_rep))
    #article_rep = test_embeddings.get_embedding(name)
    article_rep = np.reshape(article_rep, [-1]).astype('float')
...
def create_data_files(to_path):
    ...
    cnt = 0
    MAX_LEN_FILE = 100
    for paper in test_embeddings.init():
        name, abstract, embeddings, title = paper
        #abstract = paper['sum'].decode("utf-8")
        #print(type(abstract))
        #name = paper['id']
        writer.write(parse_line_to_example(name, abstract.decode('utf-8'), embeddings, title).SerializeToString())
        cnt = cnt + 1
