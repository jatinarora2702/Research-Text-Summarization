...
HParams = namedtuple('HParams',
                     ...
                     'num_softmax_samples, sent_embed_dimensions')
...
class Seq2SeqAttentionModel(object):
  ...
  def _add_placeholders(self):
    ...
    hps = self._hps
    self._articles = tf.placeholder(tf.float32,
                                    [hps.batch_size, hps.enc_timesteps, hps.sent_embed_dimensions],
                                    name='articles') 
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      encoder_inputs = tf.unpack(tf.transpose(self._articles, perm=[1,0,2])) # We unpack the inputs into one array
      ...
      with tf.variable_scope('embedding'), tf.device(self._next_device()):
      ...
      if hps.mode == 'decode' or hps.mode == 'decode_server':
        with tf.variable_scope('decode_output'), tf.device(self._next_device()):
