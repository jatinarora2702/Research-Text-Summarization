...
import data#_record_reader as data
...
class Batcher(object):
  ...
  def NextBatch(self):
    ...
    """
    enc_batch = np.zeros(
        (self._hps.batch_size, self._hps.enc_timesteps, self._hps.sent_embed_dimensions), dtype=np.float32)
  ...
  def _FillInputQueue(self):
    ...
    while True:
      ...
      while len(enc_inputs) < self._hps.enc_timesteps:
        enc_inputs.append(pad_para)  ##### TODO: Replace padid by pad embedding
  ...
  def _TextGenerator(self, example_gen):
    ...
    while True:
      e = example_gen.next()
      try:
        article_text = [x for x in e.features.feature[self._article_key].float_list.value]
