...
tf.app.flags.DEFINE_string('cell', 'mtgru', 'gru/mtgru cell to use')
...
tf.app.flags.DEFINE_integer('checkpoint_secs', 120, 'How often to checkpoint.')
...
tf.app.flags.DEFINE_integer('port', 9998, 'Port for decode server')
tf.app.flags.DEFINE_integer('epochs', None, "No of epochs")
tf.app.flags.DEFINE_integer('attn_heads', 5, "No of attention heads")
tf.app.flags.DEFINE_integer('min_input_len', 1, "")
tf.app.flags.DEFINE_bool('use_only_cpu', False, "If true, all variable are attahced to CPU")
...
def _Train(model, data_batcher):
  ...
  #if(True):
    model.build_graph()
    saver = tf.train.Saver(max_to_keep=25, keep_checkpoint_every_n_hours=0.25)
...
def main(unused_argv):
  vocab = data.Vocab(FLAGS.vocab_path, 28000)
  ...
  hps = seq2seq_attention_model.HParams(
      ...
      min_input_len=FLAGS.min_input_len,  # discard articles/summaries < than this
      num_hidden=512,  # for rnn cell
      emb_dim=512,  # If 0, don't use embedding
      max_grad_norm=2,
      num_softmax_samples=4096)  # If 0, no sampled softmax.

  

  if(hps.mode != 'decode_server'):
    ...
    model = seq2seq_attention_model.Seq2SeqAttentionModel(
        decode_mdl_hps, vocab, num_gpus=FLAGS.num_gpus)
    decoder_for_server = seq2seq_attention_decode_server.BSDecoderForServer(model, hps, vocab)
    ## Start the server object with decode_for_server
    print decoder_for_server.DecodeOne("this is a test .")
    decode_server.DecodeServer(decoder_for_server, FLAGS.port)
