...
def parse_line_to_example(article_file, abstract_file):
    article = ""
    abstract = ""
    filename = os.path.basename(article_file)
    with codecs.open(article_file, "r", "utf-8") as summa:
        #article = pre_example_process(get_doc_from_tokens(article_to_tokens(summa.read())))
        article = pre_example_process(getDocumentFromJson({"article" : summa.read()}))
    toks = article.split(" ")
    if(len(toks) < 4 or len(toks) > 200):
        return None
    arlens.append(len(toks))
    ...
    toks = abstract.split(" ")
    if(len(toks) < 4 or len(toks) > 30):
        return None
    abslens.append(len(toks))
...
def create_data_files(path, to_path):
    ...
    for i, file in enumerate(files):
        article_file = os.path.join(path,  "input", file)
        abstract_file = os.path.join(path, "output",file)
        example = parse_line_to_example(article_file, abstract_file)
        if example is None:
            continue
    ...
    print(np.histogram(arlens))
    print(np.histogram(abslens))
...
create_data_files("data_", "output")
