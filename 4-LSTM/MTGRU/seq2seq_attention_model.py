...
import seq2seq_lib
from mtgru import MTGRUCell

FLAGS = tf.app.flags.FLAGS
HParams = namedtuple('HParams',
                     'mode, min_lr, lr, batch_size, '
                     'enc_layers, taus, enc_timesteps, dec_timesteps, '
...
class Seq2SeqAttentionModel(object):
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      encoder_inputs = tf.unpack(tf.transpose(self._articles)) # We unpack the inputs into one array
      ...
      with tf.variable_scope('embedding'), tf.device('/cpu:0'):
        embedding = tf.get_variable(
            ...
            initializer=tf.truncated_normal_initializer(stddev=1e-4))  ## Create a embedding matrix of size vsize*emb_dimension
      ...
      for layer_i in xrange(hps.enc_layers):
        with tf.variable_scope('encoder%d'%layer_i), tf.device(self._next_device()):
          cell_fw = MTGRUCell(hps.num_hidden, tau=hps.taus[layer_i])
          cell_bw = MTGRUCell(hps.num_hidden, tau=hps.taus[layer_i])


          if(FLAGS.cell == "gru"):
            print("================")
            cell_fw = tf.nn.rnn_cell.GRUCell(hps.num_hidden)
            cell_bw = tf.nn.rnn_cell.GRUCell(hps.num_hidden)
      ...
      print("Here")
      with tf.variable_scope('output_projection'), tf.device('/cpu:0') :
        w = tf.get_variable(
      ...
      with tf.variable_scope('decoder'), tf.device(self._next_device()):
        ...
        cell = MTGRUCell(hps.num_hidden)

        if(FLAGS.cell == "gru"):
          cell = tf.nn.rnn_cell.GRUCell(hps.num_hidden)
        ...
        decoder_outputs, self._dec_out_state = tf.nn.seq2seq.attention_decoder(
            ...
            cell, num_heads=FLAGS.attn_heads, loop_function=loop_function,
