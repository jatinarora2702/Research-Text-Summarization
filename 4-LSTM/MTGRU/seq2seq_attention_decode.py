...
class DecodeIO(object):
  ...
  def Write(self, article, reference, decode):
    ...
    fullout = {}
    fullout['article'] = article
    fullout['abstract'] = reference
    fullout['decoded'] = decode
    strto = json.dumps(fullout)
    self._my_file.write(strto+ "\n")
    self._cnt += 1
  ...
  def ResetFiles(self):
    ...
    if self._my_file: self._my_file.close()
    ...
    self._my_file = open(
        os.path.join(self._outdir, 'all%d'%timestamp), 'w')
