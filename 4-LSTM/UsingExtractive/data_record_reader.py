...
def ExampleGen(recordio_path, num_epochs=None):
	...
	"""
	epoch = 0
	filelist = tf.train.match_filenames_once(recordio_path, name="filelist")
	...
	reader = tf.TFRecordReader(name="record_generator")
	# One can read a single serialized example from a filename
	# serialized_example is a Tensor of type string.
	_, serialized_example = reader.read(filename_queue)
	yield example_pb2.Example.FromString(serialized_example)
	'''
	while True:
		...
		for f in filelist:
			...
			while True:
				...
				# to ignore the CRC stored by serializer
				reader.read(4)
